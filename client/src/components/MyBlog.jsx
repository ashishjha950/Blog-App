...
const MyBlog = () => {
    ...
    useEffect(()=>{
        const fetchBlog = async () => {
            try {
                const response = await axios.get(`${API_URL}/blog/myblog`,{headers:{
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                }})
                setFetchedData(response.data.myPosts.map(({ writtenBy, ...rest }) => rest));
            }catch(err){
        ...
        {loading?<RotateLoader color={isDarkMode?'white':'black'}/>:(
            <div>
                <div className="py-2 px-4">
                    <h1 className={`text-3xl font-bold text-center ${isDarkMode?'text-white':'text-black'}`}>Your Blogs</h1>
